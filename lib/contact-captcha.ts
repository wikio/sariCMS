/**
 * Captcha d'image du formulaire de contact.
 *
 * Identique au captcha newsletter mais isolé : propre carnet de jetons,
 * propre route d'image, propre quota. Ainsi une attaque sur l'un ne
...
declare global {
  var __sariContactCaptcha: Map<string, Entry> | undefined;
}

function book(): Map<string, Entry> {
  const g = globalThis as typeof globalThis & { __sariContactCaptcha?: Map<string, Entry> };
  if (!g.__sariContactCaptcha) g.__sariContactCaptcha = new Map<string, Entry>();
  return g.__sariContactCaptcha;
}
...
export function issueCaptcha(): CaptchaIssue {
  ...
  return { id, imageUrl: `/api/contact/captcha/image?id=${encodeURIComponent(id)}`, expiresIn: Math.floor(TTL_MS / 1000) };
}
...
declare global {
  var __sariContactRate: Map<string, { count: number; windowStart: number }> | undefined;
}

export function rateLimited(ip: string, limit = 6, windowMs = 5 * 60 * 1000): boolean {
  const g = globalThis as typeof globalThis & {
    __sariContactRate?: Map<string, { count: number; windowStart: number }>;
  };
  if (!g.__sariContactRate) g.__sariContactRate = new Map();
  const entries = g.__sariContactRate;
