...
export const CMS_MODULES: CmsModule[] = [
  {
    ...
    fields: [
      ...
      { key: 'sku', label: 'Code produit', kind: 'text', prefix: 'SKU', placeholder: 'Laissé vide → attribué à l’enregistrement',
        hint: 'Attribué par le serveur sur un compteur partagé — deux postes ne publient plus la même référence. Une saisie manuelle est conservée telle quelle.', group: 'Catalogue' },
      { key: 'price', label: 'Prix HT', kind: 'price', wide: true, hint: 'Montant et devise sont deux champs distincts. Ajoutez une devise manquante via « Ajouter ». HT avant TVA/remises.', group: 'Catalogue' },
      { key: 'vatRate', label: 'TVA produit (%)', kind: 'number', placeholder: '19', hint: 'Vide = utilise la taxe globale/catégorie. Ex: 19, 9, 0. Prioritaire sur les taxes globales.', group: 'Commerce' },
      { key: 'vatIncluded', label: 'TVA incluse dans le prix', kind: 'toggle', hint: 'Si oui, la TVA est déjà comprise dans le prix affiché.', group: 'Commerce' },
      { key: 'discountType', label: 'Type de remise', kind: 'select', options: [{value:'percent',label:'% Pourcentage'},{value:'fixed',label:'Montant fixe (DA)'}], hint: 'Remise spécifique à ce produit, cumulable avec remise globale et coupons.', group: 'Commerce' },
      { key: 'discountValue', label: 'Valeur remise', kind: 'number', placeholder: '0', hint: 'Ex: 10 pour 10% ou 5000 DA si fixe. Par unité.', group: 'Commerce' },
      { key: 'shippingType', label: 'Frais livraison produit', kind: 'select', options: [{value:'fixed',label:'Fixe par ligne'},{value:'per_qty',label:'Par quantité'},{value:'free',label:'Gratuit'}], hint: 'Frais spécifiques à ce produit, additionnés au forfait global par zone.', group: 'Commerce' },
      { key: 'shippingFee', label: 'Montant livraison produit (DA)', kind: 'number', placeholder: '0', hint: 'Ex: 300 DA. Si par quantité, multiplié par Qté.', group: 'Commerce' },
      { key: 'weight', label: 'Poids (kg)', kind: 'number', placeholder: '0.5', hint: 'Utile si livraison au poids (mode global).', group: 'Commerce' },
      { key: 'zones', label: 'Zones disponibles', kind: 'tags', placeholder: 'Ex: DZ-16, DZ-31 ou vide = toutes', hint: 'Wilayas où ce produit est vendable. Vide = toutes zones globales actives. Ex: DZ-16,DZ-31.', group: 'Commerce' },
      { key: 'inStock', label: 'En stock', kind: 'toggle', group: 'Catalogue' },
      { key: 'stockQty', label: 'Quantité stock', kind: 'number', placeholder: '10', hint: 'Stock disponible, décrémenté à la commande si stockFinal.', group: 'Catalogue' },
      { key: 'stockFinal', label: 'Stock définitif (décrément auto)', kind: 'toggle', hint: 'Si oui, le stock est décrémenté et le produit passe hors stock à 0.', group: 'Catalogue' },
  ...
  },
  {
    ...
    fields: [
      ...
      { key: 'slug', label: 'Slug', kind: 'slug', slugFrom: 'title', group: 'Identité' , i18n: true },
      { key: 'locale', label: 'Langue', kind: 'radio', options: LOCALES, group: 'Identité' },
      { key: 'status', label: 'Statut', kind: 'radio', options: STATUS, group: 'Identité' },
      { key: 'icon', label: 'Icône Lucide', kind: 'icon', options: ICONS, hint: 'Recherchez le nom Lucide, l’icône s’affiche à côté.', group: 'Identité' , i18n: true },
      { key: 'color', label: 'Couleur', kind: 'color', group: 'Identité' , i18n: true },
  ...
  },
  {
    ...
    fields: [
      ...
      { key: 'targetAudience', label: 'Public cible', kind: 'text', placeholder: 'Ex: Professionnels de santé, Étudiants, Grand public...', hint: 'Décrivez le public visé par cet événement.', group: 'Événement', i18n: true },
  ...
  },
  {
    ...
    fields: [
      ...
      { key: 'locale', label: 'Langue', kind: 'radio', options: LOCALES, group: 'Identité' },
      { key: 'status', label: 'Statut', kind: 'radio', options: STATUS, group: 'Identité' },
      { key: 'icon', label: 'Icône', kind: 'icon', options: ICONS, group: 'Identité' , i18n: true },
      { key: 'color', label: 'Couleur', kind: 'color', group: 'Identité' , i18n: true },
      { key: 'image', label: 'Visuel', kind: 'image', group: 'Média' },
