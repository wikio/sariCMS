...
const ACCENT: Record<string, { solid: string; soft: string }> = {
  // Rien de saisi : l'habillage d'origine du bandeau — bouton principal bleu
  // dégradé (`.btn-primary`), second bouton vert SARI.
  default: {
    solid: 'btn-primary text-white',
    soft: 'bg-sari-lime text-sari-dark hover:bg-white',
  },
  lime: { solid: 'bg-sari-lime text-sari-dark hover:bg-white', soft: 'bg-white/10 text-white border border-white/40 hover:bg-white/20' },
...
export default function CTASection({ config }: CTASectionProps) {
  ...
  const accent = String(setting(config, 'accent', 'default'));
  const tone = ACCENT[accent] || ACCENT.default;
  const pattern = config?.style?.pattern !== false;
    ...
    <>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
        {primaryLabel ? (
          <Link
            href={primaryHref}
            className={`px-10 py-4 font-semibold text-lg inline-flex items-center gap-2 ${tone.solid} ${
              accent === 'default' ? '' : 'rounded-lg transition-colors'
            }`}
          >
            {primaryLabel}
        ...
        {showSecondary && secondaryLabel ? (
          <Link
            href={secondaryHref}
            className={`px-8 py-4 font-bold rounded-lg transition-colors flex items-center justify-center gap-2 ${tone.soft}`}
          >
            {secondaryLabel}
          </Link>
        ) : null}
  ...
  return (
    <BuilderBlock config={config} sectionKey="cta">
      ...
      >
        ...
        {pattern ? <div className="absolute inset-0 grid-pattern-bg opacity-10" aria-hidden /> : null}
