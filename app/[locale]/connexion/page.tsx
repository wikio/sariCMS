  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      const mockUser = {
        ...
        email: email,
        name: email.split('@')[0],
        type: source === 'carriere' ? 'candidate' : 'client',
        avatar: null
      };
      
      login(email, password, source === 'carriere' ? 'candidate' : 'client').then(success => {
        setIsLoading(false);
        if (success) {
          ...
          if (pendingAction) {
            ...
            if (action.type === 'apply') {
              router.push(`/${locale}/emplois/${action.jobId}`);
              ...
              >
                {isLoading ? (
                  <>
                    ...
                    {t('loggingIn', { defaultMessage: 'Connexion en cours...' })}
                  </>
                ) : (
                  <>
                    <LogIn className="w-5 h-5" />
                    {t('loginButton', { defaultMessage: 'Se connecter' })}
                  </>
                )}
              </button>
            </form>
