...
console.log('\n6. Le dump couvre le schéma Prisma');

// L'invariant qui manque le plus silencieusement. `sql/schema.mysql.sql` est
// généré depuis `prisma/schema.prisma` ; si un modèle est ajouté sans
// `npm run sql:schema`, le dump reste en retard. Sur une base montée à partir
// de ce dump — la voie documentée du dépôt — la table n'existe jamais, et le
// premier écran qui la lit répond 500 en production. Le client Prisma, lui, est
// à jour : rien d'autre que ce contrôle ne relie les deux.

const prismaSrc = readFileSync(resolve(HERE, '../prisma/schema.prisma'), 'utf8');
const models = new Map();
let pending = null;
for (const raw of prismaSrc.split('\n')) {
  const line = raw.trim();
  const m = /^model\s+(\w+)\s*\{/.exec(line);
  if (m) { pending = m[1]; continue; }
  const map = /^@@map\("([^"]+)"\)/.exec(line);
  if (map && pending) { models.set(map[1], pending); pending = null; }
}
check('le schéma Prisma est bien parcouru', models.size >= 30, `${models.size} modèles avec @@map`);
check(
  'aucun modèle sans @@map (une table implicite serait invisible au dump)',
  pending === null,
  pending ? `modèle ${pending} sans @@map` : '',
);

const dumped = new Set(real.keys());
const notInDump = [...models.keys()].filter((t) => !dumped.has(t));
check(
  'toute table modélisée par Prisma figure dans le dump',
  notInDump.length === 0,
  `manque au dump : ${notInDump.join(', ')} — relancer npm run sql:schema`,
);
const notInPrisma = [...dumped].filter((t) => !models.has(t));
check(
  'toute table du dump correspond à un modèle Prisma',
  notInPrisma.length === 0,
  `orpheline dans le dump : ${notInPrisma.join(', ')}`,
);
check(
  'le dump et Prisma décrivent le même nombre de tables',
  dumped.size === models.size,
  `${dumped.size} dans le dump / ${models.size} modèles`,
);

// Le nombre de colonnes, lui, n'est pas un cap arbitraire : il se déduit des
// définitions lues des deux côtés, donc il bouge avec le schéma au lieu de
// moisir comme le faisait le `> 250` du dessus.
const dumpCols = [...real.values()].reduce((n, t) => n + t.columns.length, 0);
const prismaCols = [...prismaSrc.matchAll(/^\s+\w+\s+\w+[<?!]?/gm)].length;
check(
  "le dump porte l’essentiel des colonnes du schéma",
  dumpCols > 0 && prismaCols > 0 && dumpCols >= prismaCols * 0.5,
  `${dumpCols} colonnes lues dans le dump pour ${prismaCols} champs Prisma`,
);
