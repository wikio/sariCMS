...
Le nombre de tables n'est pas une information à maintenir à la main : il se
lit dans le fichier généré, `grep -c '^CREATE TABLE' sql/schema.mysql.sql`. Le
cite-t-on ici, c'est pour situer le lecteur, pas pour faire autorité — après
chaque `npm run sql:schema`, c'est la ligne de commande qui a raison.
